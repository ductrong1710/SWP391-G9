        ...
        public async Task<ActionResult<PeriodicHealthCheckPlan>> CreatePlan(PeriodicHealthCheckPlan plan)
        {
            ...
            foreach (var form in consentForms)
            {
                if (!string.IsNullOrEmpty(form.ParentID) && !notifiedParents.Contains(form.ParentID))
                {
                    ...
                    var studentName = form.StudentID;
                    // Nếu có thể lấy profile học sinh, lấy tên thật
                    ...
                    var studentProfile = dbContext?.Profiles.FirstOrDefault(p => p.UserID == form.StudentID);
                    if (studentProfile != null)
                    {
                        studentName = studentProfile.Name;
                    }
                    var message = $"Kế hoạch kiểm tra sức khỏe định kỳ '{createdPlan.PlanName}' đã được tạo. Vui lòng xác nhận cho <b>{studentName}</b>.";
                ...
                if (form.StatusID == 2 && !string.IsNullOrEmpty(form.ReasonForDenial)) // 2: Deny
                {
                    var notification = new Notification
                    {
                        NotificationID = Guid.NewGuid().ToString(),
